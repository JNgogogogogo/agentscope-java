  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-white" data-testid="inbox-page">
      <header className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-5 py-4">
        <div><h1 className="text-xl font-semibold text-slate-900">Inbox</h1><p className="mt-1 text-sm text-slate-500">你的消息、评审与决策。</p></div>
        <div className="flex flex-wrap items-center gap-3 text-xs text-slate-500" aria-live="polite">
          {counts ? <><span><strong className="text-slate-900">{counts.unread}</strong> unread</span><span><strong className="text-amber-700">{counts.actionRequired}</strong> 项需处理</span>{counts.pendingApprovals > 0 && <Badge tone="warning">{counts.pendingApprovals} pending approvals</Badge>}</> : <span>{summary.isError ? "计数不可用" : "正在加载计数…"}</span>}
        </div>
      </header>
      <div className="grid min-h-0 flex-1 md:grid-cols-[360px_minmax(0,1fr)] xl:grid-cols-[390px_minmax(0,1fr)]">
        <section aria-label="Inbox 消息" className={cn("flex min-h-0 flex-col border-r border-slate-200", selectedId && "hidden md:flex")}>
          <div className="shrink-0 space-y-3 border-b border-slate-200 p-3">
            <div className="flex items-center justify-between gap-2">
              <div className="flex rounded-lg bg-slate-100 p-0.5" aria-label="Inbox 视图">
                {(["attention", "unread", "all"] as const).map(value => <button key={value} type="button" aria-pressed={view === value && !archived} className={cn("rounded-md px-2.5 py-1.5 text-xs font-medium", view === value && !archived ? "bg-white text-slate-900 shadow-sm" : "text-slate-500")} onClick={() => changeParams({ view: value, archived: undefined, item: undefined })}>{value === "attention" ? "需关注" : value === "unread" ? "Unread" : "全部"}</button>)}
              </div>
              <Button variant={filtersOpen || type || archived || view === "action" ? "secondary" : "ghost"} size="sm" aria-label="筛选 Inbox" aria-expanded={filtersOpen} onClick={() => setFiltersOpen(value => !value)}><Filter className="h-3.5 w-3.5" />筛选</Button>
            </div>
            {filtersOpen && <div className="grid gap-3 rounded-lg bg-slate-50 p-3">
              <label className="grid gap-1 text-xs font-medium text-slate-600">消息类型<select aria-label="Message type" className="h-9 rounded-lg border border-slate-200 bg-white px-2 text-sm" value={type} onChange={event => changeParams({ type: event.target.value || undefined, item: undefined })}><option value="">全部类型</option>{Object.entries(INBOX_TYPES).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label>
              <label className="grid gap-1 text-xs font-medium text-slate-600">状态<select aria-label="消息状态" className="h-9 rounded-lg border border-slate-200 bg-white px-2 text-sm" value={archived ? "archived" : view} onChange={event => changeParams({ view: event.target.value === "archived" ? "all" : event.target.value, archived: event.target.value === "archived" ? "true" : undefined, item: undefined })}><option value="attention">需关注</option><option value="action">需处理</option><option value="unread">未读</option><option value="all">全部消息</option><option value="archived">已归档</option></select></label>
            </div>}
            {(type || archived) && <p className="text-xs text-slate-500">{type ? INBOX_TYPES[type] || type : "All types"}{archived && " · 已归档"}</p>}
          </div>
          <div className="min-h-0 flex-1 overflow-y-auto" data-testid="inbox-list">
            {list.isPending ? <WorkLoadingRows /> : list.isError ? <WorkEmpty title="消息不可用" description="Inbox 加载失败。" action={<Button variant="outline" onClick={() => void list.refetch()}>重试</Button>} /> : items.length ? items.map((entry, index) => <InboxRow key={entry.id} item={entry} selected={selectedId === entry.id} actor={entityDisplayName(identities, entry.actor.type, entry.actor.ref)} onSelect={() => { setSelectionIndex(index); changeParams({ item: entry.id }); }} />) : <WorkEmpty title="Inbox 已清空" description="没有符合当前视图的消息。" />}
            {list.hasNextPage && <div className="p-4"><Button variant="outline" className="w-full" disabled={list.isFetchingNextPage} onClick={() => void list.fetchNextPage()}>{list.isFetchingNextPage ? "加载中…" : "加载更多"}</Button></div>}
          </div>
        </section>
        <section aria-label="消息详情" className={cn("flex min-h-0 min-w-0 flex-col", !selectedId && "hidden md:flex")}>
          {!selectedId ? <div className="flex flex-1 items-center justify-center"><WorkEmpty title="选择一条消息" description="阅读更新、评审 Issue，或处理审批。" /></div> : <>
            <div className="flex shrink-0 items-center justify-between gap-2 border-b border-slate-200 px-4 py-2">
              <div className="flex min-w-0 items-center gap-2"><Button variant="ghost" size="sm" onClick={() => changeParams({ item: undefined })}><ArrowLeft className="h-4 w-4" /><span className="md:hidden">Inbox</span></Button><span className="truncate text-xs text-slate-500">{item ? inboxTypeLabel(item) : "消息"}{item?.archived && " · 已归档"}</span></div>
              <div className="flex items-center gap-1">{item?.issueId && !item.approvalId && <Button asChild variant="ghost" size="sm"><Link to={scope.scopedPath(`/work/issues/${activeIssueId}`)}>打开 Issue ↗</Link></Button>}{item && !item.archived && !item.needsAction && <Button variant="ghost" size="sm" disabled={archive.isPending} onClick={() => archive.mutate(item.id)}><Archive className="h-3.5 w-3.5" />归档</Button>}</div>
            </div>
            {item && readErrors.has(item.id) && <div role="alert" className="flex items-center justify-between bg-amber-50 px-4 py-2 text-xs text-amber-800">无法将该消息标记为已读。<Button size="sm" variant="ghost" disabled={mark.isPending} onClick={() => mark.mutate(item.id)}>重试</Button></div>}
            {archive.isError && <p role="alert" className="bg-red-50 px-4 py-2 text-xs text-red-700">无法归档该消息。请先完成待处理的操作。</p>}
            ...
            <div className="min-h-0 flex-1 overflow-y-auto" data-testid="inbox-detail">
              {selected.isPending ? <WorkLoadingRows /> : selected.isError || !item ? <WorkEmpty title="消息不可用" description="无法加载该消息，或它在该工作区中不可见。" action={<Button variant="outline" onClick={() => void selected.refetch()}>重试</Button>} /> : item.approvalId ? <ApprovalDetail key={item.id} approvalId={item.approvalId} note={approvalNotes[item.approvalId] || ""} onNoteChange={note => setApprovalNotes(current => ({ ...current, [item.approvalId!]: note }))} onReady={markViewed} /> : activeIssueId ? <>
                <div className="border-b border-slate-100 bg-slate-50/70 px-5 py-3"><p className="text-xs font-medium text-slate-600">{item.title} · {formatRelative(item.createdAt)}</p>{item.body && <p className="mt-1 line-clamp-3 whitespace-pre-wrap text-xs leading-5 text-slate-500">{item.body}</p>}{previewIssueId && previewIssueId !== item.issueId && <Button variant="link" size="sm" className="mt-1 h-auto p-0" onClick={() => setPreviewIssueId("")}>返回通知的 Issue</Button>}</div>
...
function InboxRow({ item, selected, actor, onSelect }: { item: InboxItem; selected: boolean; actor: string; onSelect: () => void }) {
  ...
  return <button type="button" data-inbox-id={item.id} aria-current={selected ? "true" : undefined} className={cn("flex w-full gap-3 border-b border-slate-100 px-4 py-4 text-left transition hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-indigo-400", selected ? "bg-indigo-50/70 ring-1 ring-inset ring-indigo-100" : !item.read && "bg-indigo-50/20")} onClick={onSelect}>
    ...
    <span className="min-w-0 flex-1">
      ...
      <span className="mt-2 line-clamp-2 break-words text-xs leading-5 text-slate-500">{item.body || "打开查看详情"}</span>
      {item.type === "review_request" && item.needsAction && <span className="mt-2 block text-xs font-medium text-indigo-700">评审结果 →</span>}
...
function MessageDetail({ item, onReady }: { item: InboxItem; onReady: () => void }) {
  ...
  return <article className="mx-auto max-w-3xl p-6 sm:p-8"><h2 className="text-xl font-semibold">{item.title}</h2><p className="mt-2 text-xs text-slate-500">{new Date(item.createdAt).toLocaleString()}</p><div className="md-text mt-6 break-words text-sm leading-7"><ReactMarkdown>{item.body || "没有更多详情。"}</ReactMarkdown></div></article>;
}
