...
const time = (value?: string) => value ? new Date(value).toLocaleString() : '未记录';
...
function ResultContent({ value }: { value: unknown }) {
  if (value == null) return <p className="text-sm text-muted-foreground">未记录结果。</p>;
  ...
  if (!entries.length) return <p className="text-sm text-muted-foreground">执行返回了空结果。</p>;
...
function Diagnostics({ value }: { value: unknown }) {
  return <details className="mt-3"><summary className="cursor-pointer text-sm text-muted-foreground">诊断详情</summary><pre className="mt-2 max-h-96 overflow-auto whitespace-pre-wrap break-all rounded-lg bg-slate-950 p-4 text-xs text-slate-100">{JSON.stringify(value, null, 2)}</pre></details>;
}
...
export default function TaskDetailPage() {
  ...
  if (detail.isError) return <Page><p role="alert">无法加载任务：{String(detail.error)}</p></Page>;
  if (!task) return <Page>正在加载任务…</Page>;
  ...
  return <Page>
    <Link to={scope.scopedPath(task.orchestrationRunId ? `/work/executions/${task.orchestrationRunId}` : '/work/executions')} className="text-sm text-muted-foreground">← 执行</Link>
    <PageHeader title={<span className="flex flex-wrap items-center gap-3">{task.teamRole ? `${task.teamRole} 任务` : 'Agent 任务'}<Badge tone={task.status === 'completed' ? 'success' : task.status === 'failed' ? 'danger' : 'warning'}>{task.status}</Badge></span>}
      description={<><EntityIdentityText identities={identities} type="agent" entityRef={task.agentId} /> · 创建于 {time(task.createdAt)}</>}
      actions={<div className="flex gap-2">{task.status === 'failed' && <Button disabled={retry.isPending} variant="outline" onClick={() => retry.mutate()}>{retry.isPending ? '重试中…' : '重试'}</Button>}{!terminal(task.status) && <Button disabled={cancel.isPending} variant="destructive" onClick={() => cancel.mutate()}>{cancel.isPending ? '取消中…' : '取消'}</Button>}</div>} />
    ...
    <div className="mb-5 flex flex-wrap items-center gap-4 text-sm">
      ...
      <Link className="text-primary underline" to={scope.scopedPath(task.orchestrationRunId ? `/work/executions/${task.orchestrationRunId}` : '/work/executions')}>查看执行</Link>
    ...
    <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1.3fr)_minmax(0,1fr)]">
      <div className="min-w-0 space-y-5">
        <section className="rounded-xl border border-border bg-white p-5"><h2 className="mb-3 font-semibold">结果</h2>
          ...
          {finalResult == null && !terminal(task.status) ? <p className="text-sm text-muted-foreground">执行完成后会显示结果。</p> : <ResultContent value={finalResult} />}
        </section>
        <section className="rounded-xl border border-border bg-white p-5"><h2 className="mb-3 font-semibold">输入</h2>
          {!task.inputs?.length && <p className="text-sm text-muted-foreground">该任务来自 <Link className="text-primary underline" to={issuePath}>Issue 与执行上下文</Link>.</p>}
          <ol className="space-y-3">{(task.inputs || []).map(input => {
            const summary = detail.data?.inputSummaries?.find(item => item.inputId === input.id);
            return <li key={input.id} className="rounded-lg border border-border p-3"><div className="mb-2 flex justify-between text-xs text-muted-foreground"><span>输入 #{input.sequence}</span><span>{input.state}</span></div>
              {summary?.state === 'recorded' ? <ResultContent value={summary.content} /> : <p className="text-sm text-muted-foreground">{summary?.state === 'changed' ? '该输入记录之后源评论发生了变化，因此不展示为原始输入。' : '输入文本不可用。'}</p>}
      ...
      <section className="min-w-0 rounded-xl border border-border bg-white p-5"><h2 className="font-semibold">执行历史</h2>
        ...
        {!history.length && <p className="mt-3 text-sm text-muted-foreground">{attempts.isLoading ? '正在加载尝试…' : '等待运行时接收该任务。'}</p>}
        <ol className="mt-4 space-y-4">{history.map(attempt => <li key={attempt.id} className="rounded-lg border border-border p-4">
          ...
          {attempt.sessionRef && <Button asChild size="sm" variant="outline"><Link to={scope.scopedPath(`/work/sessions/${attempt.sessionRef}`)}>查看对话与事件</Link></Button>}
          {attempt.result != null && JSON.stringify(attempt.result) !== JSON.stringify(finalResult) && <details className="mt-3"><summary className="cursor-pointer text-sm">尝试结果</summary><div className="mt-2"><ResultContent value={attempt.result} /></div></details>}
