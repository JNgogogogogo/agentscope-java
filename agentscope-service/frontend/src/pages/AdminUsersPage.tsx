...
export default function AdminUsersPage() {
  ...
  return <Page><PageHeader title="Users" description="管理平台账号，以及每个人对共享空间的访问权限。" actions={<Button onClick={() => setCreating(true)}><Plus className="h-4 w-4" />Create user</Button>} />
    ...
    <div className={`grid items-start gap-6 ${selected ? 'xl:grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)]' : ''}`}><section className="overflow-x-auto rounded-xl border"><table className="w-full text-left text-sm"><thead className="bg-slate-50 text-xs text-slate-500"><tr><th className="p-4 font-medium">账号</th><th className="p-4 font-medium">平台角色</th><th className="p-4 font-medium">Status</th></tr></thead><tbody className="divide-y">{users.map(a => <tr key={a.userId} className={selected?.userId === a.userId ? 'bg-indigo-50/50' : 'hover:bg-slate-50'}><td className="p-4"><button className="text-left font-medium text-slate-900 hover:text-indigo-600" onClick={() => { const next = new URLSearchParams(params); next.set('user', a.userId); setParams(next); }}>{accountLabel(a)}</button><p className="mt-1 break-all text-xs text-slate-400">{a.userId}</p></td><td className="p-4"><Badge>{a.roles.includes('admin') ? '平台管理员' : 'User'}</Badge></td><td className="p-4"><Badge tone={a.disabled ? 'default' : 'success'}>{a.disabled ? '已停用' : '启用'}</Badge></td></tr>)}</tbody></table>{query.isLoading ? <p className="p-6 text-sm text-slate-500">正在加载账号…</p> : users.length === 0 && <p className="p-8 text-center text-sm text-slate-500">没有匹配的账号。</p>}</section>
      {selected && <UserDetail key={selected.userId} user={selected} onChanged={refresh} />}
    </div>
    <Dialog open={creating} onOpenChange={setCreating}><DialogContent size="md"><DialogHeader><DialogTitle>Create user</DialogTitle><DialogDescription>先创建账号，再为它分配所需空间的访问权限。</DialogDescription></DialogHeader><DialogBody>{creating && <CreateUserForm onCreated={password => { setCreating(false); setGenerated(password || ''); void refresh(); }} />}</DialogBody></DialogContent></Dialog>
    <Dialog open={!!generated} onOpenChange={open => { if (!open) setGenerated(''); }}><DialogContent size="md"><DialogHeader><DialogTitle>账号已创建</DialogTitle><DialogDescription>Share this initial password with the new user. It is only shown here once.</DialogDescription></DialogHeader><DialogBody><code className="block break-all rounded-lg bg-slate-50 p-4 text-lg">{generated}</code><Button className="mt-4" onClick={() => setGenerated('')}>完成</Button></DialogBody></DialogContent></Dialog>
  </Page>;
}
...
function CreateUserForm({ onCreated }: { onCreated: (password?: string) => void }) {
  ...
  return <form className="space-y-4" onSubmit={e => { e.preventDefault(); create.mutate(); }}><label className="block space-y-2 text-sm font-medium">Username<Input required maxLength={100} value={username} onChange={e => setUsername(e.target.value)} autoComplete="off" /></label><label className="block space-y-2 text-sm font-medium">初始密码<Input type="password" minLength={6} value={password} onChange={e => setPassword(e.target.value)} autoComplete="new-password" /><span className="block text-xs font-normal text-slate-500">留空则自动生成密码。</span></label><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={admin} onChange={e => setAdmin(e.target.checked)} />Platform administrator</label><ErrorNotice error={create.error} /><Button type="submit" disabled={!username.trim() || create.isPending}>{create.isPending ? '创建中…' : '创建用户'}</Button></form>;
}
...
function UserDetail({ user, onChanged }: { user: AdminUserView; onChanged: () => Promise<void> }) {
  ...
  return <aside className="space-y-5"><section className="space-y-4 rounded-xl border p-5"><div className="flex items-center gap-3"><span className="rounded-xl bg-indigo-50 p-3 text-indigo-600"><UsersRound className="h-5 w-5" /></span><div><h2 className="font-semibold">{accountLabel(user)}</h2><p className="mt-1 text-xs text-slate-500">{user.createdAt ? `Joined ${new Date(user.createdAt).toLocaleDateString()}` : user.userId}</p></div></div><h3 className="text-sm font-medium">平台角色</h3><p className="text-xs leading-5 text-slate-500">平台管理负责账号与空间的开通。空间角色与私密工作访问权限需单独分配。</p><div className="space-y-2">{['user', 'admin', 'agent_developer', 'operator'].map(r => <label key={r} className="flex items-center gap-2 text-sm"><input type="checkbox" checked={roles.includes(r)} onChange={e => setRoles(e.target.checked ? [...roles, r] : roles.filter(x => x !== r))} />{r === 'admin' ? '平台管理员' : r === 'user' ? '控制台用户' : `${r} (legacy navigation)`}</label>)}</div><ErrorNotice error={mutation.error} />{notice && <p role="status" className="text-sm text-emerald-700">{notice}</p>}<Button disabled={!roles.length || mutation.isPending} onClick={() => mutation.mutate('roles')}>保存平台角色</Button><div className="flex flex-wrap gap-2 border-t pt-4"><Button variant="outline" onClick={() => setDialog('password')}>Reset password</Button><Button variant="outline" disabled={user.userId === getUserId()} onClick={() => setDialog('status')}>{user.disabled ? '启用账号' : '停用账号'}</Button></div></section>
    <UserNamespaceAccess user={user} />
    <Dialog open={dialog !== null} onOpenChange={open => { if (!open) setDialog(null); }}><DialogContent size="md"><DialogHeader><DialogTitle>{dialog === 'password' ? '重置密码' : user.disabled ? '启用账号' : '停用账号'}</DialogTitle><DialogDescription>{dialog === 'password' ? `Set a new password for ${user.username}. Existing logins will be revoked.` : user.disabled ? 'This account can sign in again. Previous sessions remain revoked.' : 'Sign-in and existing sessions will be blocked. Historical work is retained. Transfer owned shared namespaces first.'}</DialogDescription></DialogHeader><DialogBody><div className="space-y-4">{dialog === 'password' && <Input aria-label="新账号密码" type="password" autoComplete="new-password" value={password} onChange={e => setPassword(e.target.value)} />}<ErrorNotice error={mutation.error} /><Button disabled={mutation.isPending || dialog === 'password' && password.length < 6} onClick={() => mutation.mutate(dialog === 'password' ? 'password' : 'status')}>{dialog === 'password' ? '重置密码' : user.disabled ? '启用账号' : '停用账号'}</Button></div></DialogBody></DialogContent></Dialog>
  </aside>;
}
...
function UserNamespaceAccess({ user }: { user: AdminUserView }) {
  ...
  return <section className="space-y-4 rounded-xl border p-5"><h3 className="font-semibold">空间访问权限</h3><ErrorNotice error={memberships.error || all.error} /><div className="divide-y">{assigned.map(n => <div key={n.name} className="space-y-2 py-3"><div className="flex items-center justify-between gap-2"><Link to={`/settings/namespaces/${encodeURIComponent(n.name)}`} className="text-sm font-medium hover:text-indigo-600">{n.displayName}</Link><Badge>{n.owner === user.userId ? '所有者' : n.archived ? '已归档' : '成员'}</Badge></div><RoleBadges roles={n.roles || []} />{n.groups && n.groups.length > 0 && <Link className="block text-xs text-indigo-600" to={`/settings/namespaces/${encodeURIComponent(n.name)}?tab=user+groups`}>Via user groups: {n.groups.join(', ')}</Link>}{n.kind === 'shared' && !n.archived && <Button variant="ghost" size="sm" onClick={() => setEditing(n.name)}>编辑空间访问权限</Button>}</div>)}</div>{!memberships.isLoading && assigned.length === 0 && <p className="text-sm text-slate-500">尚无空间成员身份。该用户首次登录时会自动创建个人空间。</p>}<div className="flex flex-wrap gap-2"><select aria-label="分配空间" className="h-10 min-w-0 flex-1 rounded-lg border bg-white px-3 text-sm" value={name} onChange={e => setName(e.target.value)}><option value="">选择共享空间</option>{choices.map(n => <option key={n.name} value={n.name}>{n.displayName}</option>)}</select><Button variant="outline" disabled={!name || user.disabled} onClick={() => setEditing(name)}>分配访问权限</Button></div><Dialog open={!!editing} onOpenChange={open => { if (!open) setEditing(''); }}><DialogContent size="md"><DialogHeader><DialogTitle>空间访问权限 · {editing}</DialogTitle><DialogDescription>Set {user.username}'s direct roles. User group grants are managed in Namespace user groups.</DialogDescription></DialogHeader><DialogBody>{editing && <UserGrantEditor key={editing} name={editing} user={user} onSaved={async () => { setEditing(''); setName(''); await qc.invalidateQueries({ queryKey: ['account-namespaces'] }); await qc.invalidateQueries({ queryKey: ['namespace-detail'] }); await qc.invalidateQueries({ queryKey: ['managed-namespaces'] }); scope.refreshNamespaces(); }} />}</DialogBody></DialogContent></Dialog></section>;
}
