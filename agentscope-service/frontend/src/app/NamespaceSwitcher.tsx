...
export function NamespaceSwitcher() {
  ...
  const username = getUsername() || '账号';

  return (
    <div className="border-b border-border px-3 py-3">
      <DropdownMenu.Root modal={false}>
        <DropdownMenu.Trigger asChild>
          <button type="button" aria-label="空间" className="flex w-full items-center gap-3 rounded-xl bg-slate-100/80 px-3 py-2.5 text-left transition-colors hover:bg-slate-200/70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring data-[state=open]:bg-slate-200/70">
            <Initial name={displayName} />
            <span className="min-w-0 flex-1">
              <span className="block text-[11px] text-muted-foreground">空间</span>
          ...
          <DropdownMenu.Content align="start" sideOffset={6} collisionPadding={12} className="z-50 w-72 max-w-[calc(100vw-24px)] overflow-y-auto rounded-xl border border-border bg-white p-1.5 shadow-xl outline-none max-h-[var(--radix-dropdown-menu-content-available-height)]">
            <div className="flex items-center gap-3 px-3 py-3">
              ...
              <div className="min-w-0">
                ...
                <div className="text-xs text-muted-foreground">当前登录账号</div>
              </div>
            </div>
            <DropdownMenu.Separator className="-mx-1.5 my-1 h-px bg-border" />
            <DropdownMenu.Label className="px-3 py-2 text-xs font-medium text-muted-foreground">空间</DropdownMenu.Label>
              ...
              {namespaces.map(n => (
                <DropdownMenu.RadioItem key={JSON.stringify([n.tenant, n.name])} value={JSON.stringify([n.tenant, n.name])} textValue={n.displayName || n.name} className={menuItem}>
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="block truncate text-xs text-muted-foreground">{n.kind === 'personal' ? '个人' : n.kind === 'global' ? '全局' : '共享'} · {n.tenant}/{n.name}</span>
                  </span>
            ...
            <DropdownMenu.Item className={menuItem} onSelect={() => navigate(`/settings/namespaces/${encodeURIComponent(namespace)}?${new URLSearchParams({ tenant, namespace })}`)}>
              <Settings2 aria-hidden="true" className="h-4 w-4" />空间访问权限
            </DropdownMenu.Item>
            {isAdmin() && <DropdownMenu.Item className={menuItem} onSelect={() => navigate(`/settings/namespaces?${new URLSearchParams({ create: 'true', tenant, namespace })}`)}>
              <Plus aria-hidden="true" className="h-4 w-4" />新建空间
            </DropdownMenu.Item>}
